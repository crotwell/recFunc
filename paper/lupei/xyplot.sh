...
touch $FILENAME
perl -nae 'print "$F[4] $F[5] $F[6] $F[6]\n"' ${NAME}.txt | psxy -K -P -H2 -JX6i/-8i -R${REGION} -B2/2 -Sc.1i -G255/0/0 -Ey >> $FILENAME
...
/bin/rm -f $FILENAME
touch $FILENAME
sort ${NAME}.txt | grep -v 'EARS' | grep -v 'Net' | perl -nae 'print "$. $F[5] $F[6] $F[6]\n"'  | psxy -K -P  -JX8i/-6i -R${REGION} -B2/2:"Thickness": -Sc.1i -G155 -Ey >> $FILENAME
