...
def makeMap(dataFile, outFilename, proj='M5.5i', region='-126/-114/30/50', shift='', extra='', vpvs=False, minEQ=0, backgroundGRD='/data/GMT/Grid/Crust2/thickness_180.grd', cptFile='no_green_25_50.cpt'):
    out = ' >> '+outFilename
    ...
    #crust2(outFilename, proj, region, shift, extra)
...
def makeLonVsThickBoxes(dataFile, outfilePrefix, minLat, maxLat, incLat, minEQ=0, backgroundGRD='/data/GMT/Grid/Crust2/thickness_180.grd', cptFile='no_green_25_50.cpt'):
	...
	for row in results:
	    ...
	    stalat = float(row[2])
	    if  int(row[12]) >= minEQ and stalat >= lat and stalat < (lat+incLat):
		mean = mean +float(thick)
		numMean = numMean+1
...
# label
	...
	makeMap(dataFile, outFilename, 'M6i', mapRegion,'-Y6.5i','-O', minEQ=minEQ)
	ps2pdf(outFilename)
...
#makeBigMap('allWestCoast.csv', 'mapAll.ps', 'M5.5i','-126/-114/32/49' )
#ps2pdf('mapAll.ps')
...
#makeLonVsThickBoxes('TAresult.csv', 'TAlonBox_', 32, 49, 2, minEQ=5)
