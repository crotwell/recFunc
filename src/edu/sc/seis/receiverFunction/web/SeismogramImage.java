...
import edu.sc.seis.receiverFunction.DataSetRecFuncProcessor;
...
public class SeismogramImage extends HttpServlet {
        ...
        try {
            ...
            if(RevUtil.get("H", req, null) != null && RevUtil.get("vpvs", req, null) != null) {
                ...
                StationResult staResult = new StationResult(result.channels[0].get_id().network_id,
                                                            ...
                                                            new QuantityImpl(h, UnitImpl.KILOMETER),
                                                            vpvs,
                                                            crust2.getStationResult(result.channels[0].my_site.my_station)
                                                                    .getVp(),
                                                            null);
                Sampling samp = new SamplingImpl(10, new TimeInterval(1, UnitImpl.SECOND));
                SimpleSynthReceiverFunction synth = new SimpleSynthReceiverFunction(staResult, samp, 2048);
                DistAz distAz = new DistAz(result.channels[0], result.prefOrigin);
                Arrival[] arrivals = TauPUtil.getTauPUtil().calcTravelTimes(distAz.getDelta(), 0, new String[] {"P"});
                float flatRP = (float)arrivals[0].getRayParam() / 6371;
                MicroSecondDate pTime = new MicroSecondDate(result.prefOrigin.origin_time);
                pTime = pTime.add(new TimeInterval(arrivals[0].getTime(), UnitImpl.SECOND));
                //pTime = pTime.subtract(RecFunc.DEFAULT_SHIFT);
                pTime = dss[0].getBeginMicroSecondDate();
                logger.debug("synth begin="+pTime+"  radial begin="+dss[0].getBeginMicroSecondDate());
                LocalSeismogramImpl synthRadial = synth.calculate(flatRP,
                                                                  result.radial.begin_time,
                                                                  RecFunc.DEFAULT_SHIFT,
                                                                  dss[0].getChannelId(),
                                                                  result.config.gwidth);
                synthDSS = new MemoryDataSetSeismogram(synthRadial);
                dss[0].getDataSet().addDataSetSeismogram(synthDSS, new AuditInfo[0]);
                disp.get(dss[0]).add(new DataSetSeismogram[] {synthDSS});
                
                logger.debug(" max amp data: "+new Statistics(((MemoryDataSetSeismogram)dss[0]).getCache()[0].get_as_floats()).max());
                logger.debug("max amp synth: "+new Statistics(synthDSS.getCache()[0].get_as_floats()).max());
            }
            logger.debug("before shale data: "+disp.getTimeConfig().getTime(dss[0]));
            logger.debug("            synth: "+disp.getTimeConfig().getTime(synthDSS));
            ...
            disp.getTimeConfig().shaleTime(0, window.divideBy(mstr.getInterval()).getValue(SEC_PER_SEC));
            logger.debug("after shale data: "+disp.getTimeConfig().getTime(dss[0]));
            logger.debug("            synth: "+disp.getTimeConfig().getTime(synthDSS));
            ...
            System.out.println("No waveforms found for id " + req.getParameter("rf"));
            writer.write("<html><body><p>No waveforms found for id " + req.getParameter("rf") + "</p></body></html>");
            writer.flush();
        } catch(Exception e) {
    ...
    public DataSetSeismogram[] getDSS(CachedResult stack) {
        ...
        MemoryDataSetSeismogram radialDSS = new MemoryDataSetSeismogram(radial, "radial RF");
        DataSet dataset = new MemoryDataSet("temp", "Temp Dataset for " + radialDSS.getName(), "temp", new AuditInfo[0]);
        dataset.addDataSetSeismogram(radialDSS, emptyAudit);
        String channelParamName = StdDataSetParamNames.CHANNEL + ChannelIdUtil.toString(radial.channel_id);
        ...
        MemoryDataSetSeismogram tangentialDSS = new MemoryDataSetSeismogram(tangential, "tangential RF");
        dataset.addDataSetSeismogram(tangentialDSS, emptyAudit);
        channelParamName = StdDataSetParamNames.CHANNEL + ChannelIdUtil.toString(tangential.channel_id);
        dataset.addParameter(channelParamName, stack.channels[0], emptyAudit);
        // Z
        LocalSeismogramImpl zSeis = (LocalSeismogramImpl)stack.original[0];
        MemoryDataSetSeismogram zDSS = new MemoryDataSetSeismogram(zSeis, zSeis.getName());
        dataset.addDataSetSeismogram(zDSS, emptyAudit);
        channelParamName = StdDataSetParamNames.CHANNEL + ChannelIdUtil.toString(stack.channels[0].get_id());
        dataset.addParameter(channelParamName, stack.channels[0], emptyAudit);
        // a horizontal
        LocalSeismogramImpl aSeis = (LocalSeismogramImpl)stack.original[1];
        MemoryDataSetSeismogram aDSS = new MemoryDataSetSeismogram(aSeis, aSeis.getName());
        dataset.addDataSetSeismogram(aDSS, emptyAudit);
        channelParamName = StdDataSetParamNames.CHANNEL + ChannelIdUtil.toString(stack.channels[1].get_id());
        ...
        MemoryDataSetSeismogram bDSS = new MemoryDataSetSeismogram(bSeis, bSeis.getName());
        channelParamName = StdDataSetParamNames.CHANNEL + ChannelIdUtil.toString(stack.channels[2].get_id());
