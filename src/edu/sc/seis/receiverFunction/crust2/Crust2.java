...
public class Crust2 {
    ...
    private void parseTypes() throws IOException {
        InputStream inStream = getClass().getClassLoader().getResourceAsStream(DataPrefix+"CNtype2_key.txt");
    ...
    private void parseLatLonModel() throws IOException {
        InputStream inStream = getClass().getClassLoader().getResourceAsStream(DataPrefix+"CNtype2.txt");
        BufferedReader in = new BufferedReader(new InputStreamReader(inStream));
        ...
        ArrayList values = split(in.readLine());
            ...
            for (int i = 0; i < longitudes.length; i++) {
                ...
                // lat values in file are 90, 88, 86 but center points are 89, 87, 85, so -1
                // lon values in file are -180, -178, -176 but center points are -179, -177, -175, so +1
