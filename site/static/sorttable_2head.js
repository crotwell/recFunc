...
function ts_makeSortable(table) {
    ...
    // We have a second row: assume it's the header, and make its contents clickable links
    var sortedColumn = secRow.cells[0];
    for (var i=0;i<secRow.cells.length;i++) {
        var cell = secRow.cells[i];
...
function ts_resortTable(lnk) {
    ...
    for (j=2;j<table.rows.length;j++) { newRows[j-1] = table.rows[j]; }
