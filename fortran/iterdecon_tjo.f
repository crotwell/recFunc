...
c
      ...
      real amps(MAXG)
      integer shifts(MAXG)
      character*256 numerator, denominator, outfile
...
c
      write(stdout,*) ' '
      write(stdout,*) 'Program iterdeconfd - Version 1.0X, 1997-98'
      write(stdout,*) 'Chuck Ammon, Saint Louis University'
      write(stdout,*) ' '
c
c     read in the names of the input files
c      
   9  call zero(f,MAXPTS)
      call zero(g,MAXPTS)
      read(stdin,'(a)',END=99) numerator
...
c**************************************surya*******
        call setfhv('USER7',d_error,nerr)
	call setfhv('USER8',float(nshifts),nerr)
	call setfhv('USER9',fit,nerr)
c**************************************surya**********
      call wsac0(outfile(1:lenout),xdummy,p,nerr)
c
...
*
      ...
      real gwidth, gwidth_factor, gauss, sum
